...
class Data_Class:
    ...
    def plot_save_surface(self, calibrated_iv, date_today):
        ...
        K_vals, T_vals = np.meshgrid(
            np.linspace(min(moneyness), max(moneyness), 100),
            np.linspace(min(self.T), max(self.T), 100)
        )

        # Interpolating the calibrated surface
        calibrated_iv_interp = griddata(
            (moneyness, self.T), calibrated_iv, (K_vals, T_vals), method='cubic'
        )

        # Interpolating the original implied volatilities
        imp_vol_interp = griddata(
            (moneyness, self.T), self.market_vol, (K_vals, T_vals), method='cubic'
        )
        ...
        ax1.set_title('Calibrated Heston IV Surface', y=0.95, fontsize=16)
        ...
        ax1.zaxis.set_major_locator(MultipleLocator(10))
        ...
        ax2.set_title('Market IV Surface', y=0.95, fontsize=16)
        ...
        ax2.zaxis.set_major_locator(MultipleLocator(10))
